...
if "%~1"=="" (
  echo Usage: simple-post-json.bat config.json [output.pdf]
...
echo Sending %JSON_FILE% to generate PDF...

REM Simple curl command with minimal options - using @ syntax for file upload
curl -X POST "http://127.0.0.1:4002/upload-json" -F "jsonFile=@%JSON_FILE%" -o "%OUTPUT_FILE%"
